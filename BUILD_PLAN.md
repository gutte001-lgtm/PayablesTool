...
### Phase 4.5 — AP Dueness Classification (`claude/phase-4-5`)
Introduces the two-dimensional bill model — `obligation_type` (`ordinary_ap` default / `debt_service` / `not_real_ap`) × `due_state` (`due` / `not_due`) — on `bill_metadata`, plus `invoice_due_date` (contractual, QB-locked, audit-logged on sync changes) and `expected_payment_date` (editable CFO cash-forecast field). Sync auto-defaults `obligation_type=debt_service` for bills hitting a known liability account (**Decathlon's GL 26110 `NOTES PAYABLE:Decathlon Alpha IV, L.P.`** — the sole confirmed liability account; `sync.LIABILITY_ACCOUNT_NUMBERS`, extended during triage) and **auto-promotes `debt_service` `not_due→due` on `invoice_due_date`** (once per sync pass, idempotent); `ordinary_ap` is **never** auto-promoted (a human flips the trigger). The pay-run builder is **hard-fenced at the SQL layer (`payruns.candidate_bills`) to `due` cells only** (`ordinary_ap+due`, `debt_service+due`); `not_real_ap` and any `not_due` bill cannot be added even by a forced POST — the phase's key operational safety guard. Adds the eight `bill_metadata` columns + the `classification_audit` change trail + the extensible `classification_reason_lookup` (mirrors `status_pill_lookup`), a bill-detail classification panel with change history (role-gated: controller+cfo own `obligation_type`, ap_clerk can flip `due_state`), `/bills` badges + filters, a controller admin route to extend the reason dropdown, and a keyboard-driven `/triage` heuristic suggester (deterministic, no LLM) for the pre-go-live pass. Every classification change and every sync-driven `invoice_due_date` update writes a `classification_audit` row. `init_db.py` builds the schema for fresh DBs (`PHASE_4_5_SCHEMA` + the new `bill_metadata` columns); `migrations/006_phase_4_5.py` upgrades the live DB idempotently (ALTER-adds the eight columns, backfilling existing bills to `ordinary_ap`/`not_due`; dates populate on the next sync — run a sync after migrating). Tests: `test_phase_4_5.py`. **Reporting alignment** (`/summary` two-headline redesign, export filtering, BOD AP report, `/classifications` CFO page) is **Phase 4.6**; **notes lifecycle + CFO Briefing** is **Phase 4.7**.

### Phase 4.6 — Reporting alignment (`claude/phase-4-6`)
Consumes the 4.5 model on the read side. **`/summary` redesigned into three classification sections**: *Right Now AP* (`ordinary_ap`+`due`, aged by `invoice_due_date`, with a by-category breakdown — the board-defensible number), *Pipeline / Contingent* (`ordinary_ap`+`not_due` plus `not_real_ap`, bucketed by `expected_payment_date`), and *Debt Service* (all `debt_service`, due+not_due installments, **never folded into AP totals**). **Export fence**: `exports.py` `_LINE_SQL` now filters check-run exports (CFO + CEO) to the same predicate as the pay-run fence (`obligation_type IN (ordinary_ap,debt_service) AND due_state='due'`) — a `not_due`/`not_real_ap` bill can't appear on an export even if reclassified after lock. **New BOD AP report** (`/summary/bod-ap-report.xlsx`, controller+cfo): `ordinary_ap` aging in Due + Not-Due sections, excludes `debt_service`/`not_real_ap`. **New `/classifications` CFO review page** (controller+cfo, read-only): changes-since-last-visit (per-user marker is an `audit_log` `classifications_reviewed` row — **no new table**), open `not_real_ap` items, pending trigger flips (`ordinary_ap`+`not_due` past `expected_payment_date`), and a searchable flat list. Also: the **triage deposit heuristic tightened** (gates on a linked vendor credit, not mere partial payment — a partially-paid COGS bill now suggests `ordinary_ap` with a "possible deposit? verify" flag, fixing the Candela false positive); **debt-service detection now runs on the sync update path too**, hard-guarded by `classified_by IS NULL` so it never overwrites a human classification. No schema change, no migration (read-side phase). Tests: `test_phase_4_6.py`. **Legacy `classification` enum**: recommend retiring it into the 2-D model in a later dedicated step (it holds no real data — 1 `Real` row, rest NULL); kept as-is here, both fences compose harmlessly. **Notes lifecycle + CFO Briefing** is **Phase 4.7**.
